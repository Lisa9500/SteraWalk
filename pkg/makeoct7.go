...
func MakeOct7(XY [][]float64, order map[string]int) (cord [][]float64,
	...
	if (flagEdge2to3 == true) && (flagEdge3to4 == true) {
		...
		// divLine2a1 := DistVerts(XY[num2][1], XY[num2][0], int2a1X, int2a1Y)
		// divLine2b2 := DistVerts(XY[num2][1], XY[num2][0], int2b2X, int2b2Y)
		...
		// divLine2a3 := DistVerts(XY[num2][1], XY[num2][0], int2a3X, int2a3Y)
		// divLine2b4 := DistVerts(XY[num2][1], XY[num2][0], int2b4X, int2b4Y)
		...
		if distL1D2a < 1.0 && distL2D1a < 1.0 {
			// ２つの四角形に分割する．なお，中間点となるL点は使用しない．
			rect1name = []string{"R3", "L1", "R1", "R2"}
			// log.Println("rect1name", rect1name)
			rect2name = []string{"R6", "L2", "R4", "R5"}
			// log.Println("rect2name", rect2name)
			// 辞書の中身に従ってリストの座標データで四角形を作る
			rect1List = MakeRectList(XY, order, rect1name)
			log.Println("rect1List=", rect1List)
		...
		log.Println("rect1name", rect2name)
		...
		// // R1点とD2b点の間の距離
		// noR1 := (num1 + 1) % nodOct
		// distR1D2b := DistVerts(XY[noR1][0], XY[noR1][1], int2b4X, int2b4Y)
		...
		// // R3点とD1a点の間の距離
		// noR3 := (num1 + 3) % nodOct
		// distR3D1a := DistVerts(XY[noR3][0], XY[noR3][1], int1a1X, int1a1Y)
		...
		// // R4点とD1b点の間の距離
		// noR4 := (num2 + 1) % nodOct
		// distR4D1b := DistVerts(XY[noR4][0], XY[noR4][1], int1b4X, int1b4Y)
		...
		// // R6点とD2a点の間の距離
		// noR6 := (num2 + 3) % nodOct
		// distR6D2a := DistVerts(XY[noR6][0], XY[noR6][1], int2a1X, int2a1Y)
