...
func MakeOct3(XY [][]float64, order map[string]int) (cord [][]float64, rect1List [][]float64, rect2List [][]float64, rect3List [][]float64) {
	...
	// 対向する辺は，L1点から４つ目と５つ目の点で結ばれる線分
	// 対向する辺の座標ペア
	taikoCord1a := make([][]float64, 2)
	num1P4a := (num1 + 4) % nodOct
	taikoCord1a[0] = XY[num1P4a]
	num1P5a := (num1 + 5) % nodOct
	...
	// 対向する辺は，L2点から３つ目と４つ目の点で結ばれる線分
	// 対向する辺の座標ペア
	taikoXYb := make([][]float64, 2)
	num2N3b := (num2 + 3) % nodOct
	taikoXYb[0] = XY[num2N3b]
	num2N4b := (num2 + 4) % nodOct
