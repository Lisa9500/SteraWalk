...
func TriVert(n int, XY [][]float64) (extLst []float64,
	...
	for i := 0; i < n; i++ {
		...
		xn := XY[(i+1)%n][0]
		yn := XY[(i+1)%n][1]
		// log.Println(i, "xs=", xs) // Ctrl+/
		// log.Println(i, "ys=", ys) // Ctrl+/
		// log.Println(i, "xp=", xp) // Ctrl+/
		// log.Println(i, "yp=", yp) // Ctrl+/
		// log.Println(i, "xn=", xn) // Ctrl+/
		// log.Println(i, "yn=", yn) // Ctrl+/
		...
		// log.Println(i, "ベクトルAx", ax) // Ctrl+/
		// log.Println(i, "ベクトルAy", ay) // Ctrl+/
		// log.Println(i, "ベクトルA", al)  // Ctrl+/
		// ベクトルBのX座標の差分
		bx := xn - xs
		// ベクトルBのY座標の差分
		by := yn - ys
		...
		// log.Println(i, "ベクトルBx", bx) // Ctrl+/
		// log.Println(i, "ベクトルBy", by) // Ctrl+/
		// log.Println(i, "ベクトルB", bl)  // Ctrl+/
